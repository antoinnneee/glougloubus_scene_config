...
| 2 | Animations via keyframes (UI mini-timelines) | ⏳ Non démarré (générateur déjà branché en backend, mais pas l'UI par track) |
| 3 | Panneau de calques | ⏳ Non démarré |
| 4 | Multi-sélection + groupes | ⏳ Non démarré |
| 5 | Transforms avancés (rotation, flip, alignement) | ⏳ Non démarré |
...
**Objectif** : un panneau listant les objets, avec drag pour réordonner (z-order), lock, hide, rename.
...
### Phase 4 — Multi-sélection + groupes

**Objectif** : sélectionner plusieurs objets pour les déplacer/grouper/aligner.
...
### Phase 5 — Transforms avancés

**Objectif** : rotation, flip, alignement, distribution.

**Ajouts au schéma** :
- track `rotation` (en degrés, 0 = aucun) — déjà prévu dans le schéma cible
- propriétés statiques `flipX`, `flipY` (booleans)

**UI** :
- Handle de rotation au-dessus du bounding box (style classique)
- Boutons dans la sheet (onglet "Props" enrichi) :
  - Flip horizontal / vertical
  - Align left / center-h / right
  - Align top / center-v / bottom
  - Distribute horizontally / vertically (besoin de ≥3 sélectionnés)
- Snap aux objets : pendant un drag, magnétisme sur les bords/centres des autres objets non-sélectionnés (toggle dans Props)
