...
| 6 | Timeline visuelle complète | ✅ **Fait** |
...
### Phase 6 — Timeline visuelle complète ✅ FAIT

**Objectif (atteint)** : la frame strip est remplacée par une vraie timeline scrubbable avec visualisation des keyframes de l'objet sélectionné, ligne par track.

**Livré** :
- ✅ `modules/timeline-global.js` : rendu stateless de la règle + lanes par track.
- ✅ Règle horizontale avec ticks majeurs/mineurs et labels (cadence adaptative selon `frameCount`). Click/drag = scrub.
- ✅ Playhead vertical (ligne accent) au-dessus de la règle ET dans chaque lane (ligne fine bleue).
- ✅ Une lane par track animable de l'objet sélectionné (x, y, size/scale, rotation, color, opacity, x1/y1/x2/y2 selon type).
- ✅ Dots cliquables/draggable par keyframe :
  - tap = seek vers la frame du kf
  - drag horizontal = déplace le kf (`onMoveKf`)
  - clic-droit / contextmenu = supprime (`onRemoveKf`)
- ✅ Long-press sur une lane vide = ajoute un kf à la frame cliquée avec la valeur courante (`onAddKf`).
- ✅ Mode "Pas de sélection" : message hint, pas de lanes (la règle reste utilisable pour scrubber les frames).
- ✅ Playback met à jour seulement la position du playhead (pas de DOM rebuild) pour rester fluide à 60 fps.

**Limitations connues** :
- Plus de drag-to-reorder de frames (était sur les thumbnails). Si besoin, ré-ajouter via menu contextuel sur la règle.
- Pas de zoom horizontal sur la timeline pour les très long projets (>200 frames). Les ticks deviennent denses mais restent lisibles.
- Une seule lane par prop : on n'affiche pas les easings différemment.
